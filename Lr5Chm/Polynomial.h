...
namespace luMath
{
	...
	//template <class T> class Polynomial<Fraction<T>>
	//{
	//private:
	//	static unsigned s_idCounter;
	//	unsigned m_id;
	...
	//	Fraction<T>* m_coeff;
	//public:
	//	explicit Polynomial(unsigned length = 1, Fraction<T>* coeff = nullptr)
	//		: m_id{ s_idCounter++ }, m_length{ length }
	//	{
	//		if (m_length < 0)
	//			throw  std::invalid_argument("Степень полинома отрицательна.");

	//		m_coeff = new Fraction<T>[m_length];
	//		if (coeff)
	//		{
	//			for (unsigned i = 0; i < m_length; i++)
	//				m_coeff[i] = coeff[i];
	...
	//	Polynomial(std::initializer_list<Fraction<T>> list)
	//		: m_id{ s_idCounter++ }
	//	{
	//		m_length = static_cast<unsigned>(list.size());
	//		m_coeff = new Fraction<T>[m_length];
	...
	//			m_coeff[count++] = element;
	//	}
	//	Polynomial(const Polynomial<Fraction<T>>& polynomial)
	//		: Polynomial<Fraction<T>>(polynomial.m_length, polynomial.m_coeff)
	//	{	}
	...
	//	const Polynomial& operator+=(const Polynomial<Fraction<T>>& polynomial)
	//	{
	//		unsigned i = 0;
	//		if (m_length >= polynomial.m_length)
	//		{
	...
	//		else
	//		{
	//			T* temp_array = new Fraction<T>[polynomial.m_length];
	//			for (; i < m_length; i++)
	//				temp_array[i] = m_coeff[i] + polynomial.m_coeff[i];
	//			delete[] m_coeff;
	//			m_length = polynomial.m_length;
	...
	//	const Polynomial& operator=(Fraction<T> number)
	//	{
	//		if (m_length != 1)
	//		{
	//			delete[] m_coeff;
	//			m_length = 1;
	//			m_coeff = new Fraction<T>[1];
	//		}

	//		m_coeff[0] = number;
	//		return *this;
	//	}
	//	const Polynomial& operator=(Polynomial<Fraction<T>>&& polynomial) noexcept
	//	{
	//		if (this == &polynomial)
	//			return *this;
	//		std::swap(m_length, polynomial.m_length);
	...
	//	double operator()(Fraction<T> arg) const
	//	{
	//		Fraction<T> res = 0.0;
	//		for (unsigned i = 0; i < m_length; i++)
	//			res += m_coeff[i] * std::pow(arg, i);
	...
	//		return m_coeff[index];
	//	}
	//	const Fraction<T>& operator[](int index) const
	//	{
	//		if (index >= static_cast<int>(m_length) || static_cast<int>(m_length) + index < 0)
	//			throw std::invalid_argument("Индекс члена полинома больше количества членов");
	//		else if (index < 0)
	...
	//	friend std::ostream& operator<<(std::ostream& out, const Polynomial<Fraction<T>>& polynomial)
	//	{
	//		std::streamsize width = out.width();
	...
	//	friend std::istream& operator>>(std::istream& in, const Polynomial<Fraction<T>>& polynomial)
	//	{
	//		for (unsigned i = 0; i <= polynomial.m_length; ++i)
	//			in >> polynomial.m_coeff[i];
	//		return in;
	...
	//	friend Polynomial operator*(const Polynomial<Fraction<T>>& p1, const Polynomial<Fraction<T>>& p2)
	//	{
